...
#if 0
static INT32 mk_imx8mm_image(imx8mm_header_t *ptImx8mmHead, INT8 *pcBinName, INT8 *pcOutputName, UINT32 uiBinOffest, UINT8 ucIsNeedHead);
#endif

static INT32 mk_imx8mm_image(INT32 iFd, void *pucBuf, UINT32 uiSize);
...
INT32 imx8mm_mkimage(int argc , char **argv)
{
    int c = 0;
    int iOptIndex=0;
    ...
    memset(&tImx8mmHead, 0x00, sizeof(tImx8mmHead));
    ...
    while(1)
    {
       ...
       switch(c)
       {
           ...
           case 'v':
           {
               printf("current version:V%d.%d\n",MK_IMX8MM_MAJOR_VERSION,MK_IMX8MM_MINOR_VERSION);
               break;
           }
    ...
    tFlashHead.self = uiEntryAddr - sizeof(imx8mm_header_t) - ALIGN(uiDcdSize, 64);
    tFlashHead.boot_data_ptr = tFlashHead.self + offsetof(imx8mm_header_t, boot_data);
    if (uiDcdSize)
    {
       tFlashHead.dcd_ptr = tFlashHead.self + sizeof(imx8mm_header_t);
    }
    else
    {
    ...
    if(uiPluginFlag == 0)
    {
       ...
       tBootData.size = ALIGN(uiIvtOffset + sizeof(imx8mm_header_t) + ALIGN(uiDcdSize,64) 
                        + BIN_TOTAL_SIZE, 0x200);//+ tSbuf.st_size, 0x200);
    }
    tBootData.plugin = uiPluginFlag;

    memcpy(&tImx8mmHead.boot_data, &tBootData, sizeof(tBootData));

    tFlashHead.csf = tBootData.start + tBootData.size;
    memcpy(&tImx8mmHead.fhdr, &tFlashHead, sizeof(tFlashHead));
	...
	memcpy(pucBuf, &tImx8mmHead, sizeof(tImx8mmHead));
...
static INT32 mk_imx8mm_image(imx8mm_header_t *ptImx8mmHead, INT8 *pcBinName, INT8 *pcOutputName, UINT32 uiBinOffest, UINT8 ucIsNeedHead)
{
    ...
    if (NULL == ptImx8mmHead)
    {
        printf("ptImx8mmHead is NULL\n");
        return -1;
    }
	...
	if (1 == ucIsNeedHead)
	{
		write(iOutFd, ptImx8mmHead, sizeof(*ptImx8mmHead));
	}
